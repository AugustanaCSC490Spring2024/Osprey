...
class FriendsFirestoreService {
  ...
  Future<void> requestFriend(String currentUser, String requestedFriend) async {
    ...
    if (doc1.exists&&!doc2.exists) {
      users.doc(currentUser).collection("friends").doc(requestedFriend).set({
        "date":Timestamp.now(),
        "status":"requested"
      });
      users.doc(requestedFriend).collection("friends").doc(currentUser).set({
        "date":Timestamp.now(),
        "status":"pending"
      });
    }
  ...
  Future<void> acceptFriend(String person, String friend) async {
    users.doc(person).collection("friends").doc(friend).update({"status":"accepted"});
    users.doc(friend).collection("friends").doc(person).update({"status":"accepted"});
  }
